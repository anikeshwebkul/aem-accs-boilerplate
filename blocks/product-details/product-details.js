...
import { applyPdpGate } from '../../scripts/verification/gate-ui.js';
...
export default async function decorate(block) {
  ...
  // Customer-verification gate (fails open; no-op when not gated).
  // Pass the product type when available so type-based exemptions work.
  // The PDP model does not expose categories, so categoryIds is left empty;
  // SKU-level and rules-off gating still function correctly.
  if (product?.sku) {
    applyPdpGate({
      sku: product.sku,
      item: {
